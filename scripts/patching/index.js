...
import { d20Roll } from "../../../../systems/sns/module/dice.js";
import { sns, i18n, Utils } from "../utils/index.js";
...
export function patchCoreFunctions() {
	if (!libWrapper.is_fallback && !libWrapper.version_at_least?.(1, 4, 0)) {
		Hooks.once("ready", () => {
			...
			ui.notifications.error(i18n("brSnS.error.libWrapperMinVersion", { version }));
		});
	...
	libWrapper.register("betterrollsSnS", `${actorProto}.rollSkill`, actorRollSkill, "MIXED");
	libWrapper.register("betterrollsSnS", `${actorProto}.rollAbilityTest`, actorRollAbilityTest, "MIXED");
	libWrapper.register("betterrollsSnS", `${actorProto}.rollAbilitySave`, actorRollAbilitySave, "MIXED");
}
...
function override(target, fn) {
	libWrapper.register("betterrollsSnS", target, fn, "OVERRIDE", {chain: true});
}
...
async function itemRollAttack(defaultRoll, options) {
	...
	const flags = this.actor.data.flags.sns || {};
	if ( !this.hasAttack ) {
	  throw new Error("You may not place an Attack Roll with this Item.");
	}

	let title = `${this.name} - ${game.i18n.localize("SNS.AttackRoll")}`;
	...
	const rollConfig = mergeObject({
		...
		messageData: {
			...
			"flags.sns.roll": { type: "attack", itemId: this.id }
		}
	}, options);
...
async function actorRollSkill(original, skillId, options) {
	...
	return CustomRoll._fullRollActor(this, i18n(sns.skills[skillId]), roll);
}
...
async function actorRollAbilityTest(original, ability, options) {
	...
	const label = `${i18n(sns.abilities[ability])} ${i18n("brSnS.chat.check")}`;
...
async function actorRollAbilitySave(original, ability, options) {
	...
	const label = `${i18n(sns.abilities[ability])} ${i18n("brSnS.chat.save")}`;
	return CustomRoll._fullRollActor(this, label, roll);
}
